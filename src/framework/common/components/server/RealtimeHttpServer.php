...
//        $this->_rtc->on('client-context-ack', [$this, 'onClientContextAck']);
    ...
    //test
    private function getLo($link)
    {
        $client = new \GuzzleHttp\Client();
        $outputFile = Utils::createTempFile(sys_get_temp_dir(), 'IMG');
        try {
            $response = $client->request('GET', $link, ['sink' => $outputFile]);
            ...
            Log::writeToLog($photo->getFile(),'Photo');
            return $photo->getFile();
        } catch (\Exception $e) {
            Log::writeToLog($e->getMessage(),'Error');
        } finally {
            @unlink($outputFile);
        }
    }
    ...
    protected function _startHttpServer()
    {
        ...
        $socket = new \React\Socket\Server(self::HOST.':'.$this->port, $this->_loop);
